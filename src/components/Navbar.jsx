...
function DropdownMenu({ items }) {
  return (
    <div className="absolute top-[calc(100%+10px)] left-0 w-52 bg-[#13201b] rounded-xl shadow-xl border border-emerald-900/40 py-1.5 z-50 animate-fadeIn">
      {items.map((item) => (
        <Link
          key={item.to}
          to={item.to}
          className="flex flex-col px-4 py-2.5 mx-1 rounded-lg hover:bg-emerald-500/10 transition-colors"
        >
          <span className="text-sm font-medium text-stone-100">
            {item.label}
          </span>
          {item.sub && (
            <span className="text-xs text-stone-500 mt-0.5">{item.sub}</span>
          )}
        </Link>
      ))}
    </div>
  ...
  return (
    <>
      ...
      <header ref={navRef} className="sticky top-0 z-50">
        <nav className="bg-[#0a0e0d] border-b border-emerald-900/30 px-4 md:px-8">
          <div className="max-w-6xl mx-auto flex items-center justify-between h-16">
            ...
            <div className="hidden md:flex items-center gap-1">
              <Link
                to="/"
                className="text-stone-300 hover:text-[#d4af37] hover:bg-emerald-500/10 text-sm px-3.5 py-2 rounded-lg transition-colors"
              >
                Home
              </Link>
              ...
              <div className="relative">
                <button
                  onClick={() => toggle("hajj")}
                  className="flex items-center gap-1.5 text-stone-300 hover:text-[#d4af37] hover:bg-emerald-500/10 text-sm px-3.5 py-2 rounded-lg transition-colors"
                  aria-expanded={openMenu === "hajj"}
                >
                  Hajj <ChevronIcon open={openMenu === "hajj"} />
                </button>
            ...
            <div className="flex items-center gap-3">
              <Link
                to="/contact"
                className="hidden md:inline-flex bg-[#d4af37] hover:bg-[#c9a449] text-[#0a0e0d] font-semibold text-sm px-4 py-2 rounded-lg transition-colors"
              >
                Book Now
              </Link>
              <button
        ...
        {/* Mobile menu */}
        {mobileOpen && (
          <div className="md:hidden bg-[#0a0e0d] border-t border-emerald-900/30 px-5 pb-5">
            <Link
              to="/"
              className="block text-stone-300 text-[15px] py-3 border-b border-white/[0.08]"
            >
                ...
                <div className="pb-2 space-y-0.5">
                  {hajjPackages.map((p) => (
                    <Link
                      key={p.to}
                      to={p.to}
                      className="block px-2 py-2 rounded-lg text-stone-400 text-sm hover:text-[#d4af37] hover:bg-emerald-500/10 transition-colors"
                    >
                      {p.label}
                    </Link>
                  ))}
            ...
            <Link
              to="/book"
              className="block mt-4 text-center bg-[#d4af37] hover:bg-[#c9a449] text-[#0a0e0d] font-semibold text-[15px] py-3 rounded-xl transition-colors"
            >
              Book Now
            </Link>
          </div>
